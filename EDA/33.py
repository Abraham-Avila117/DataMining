...
# maximized binning 
plt.figure(figsize=(8, 5))
sns.histplot(x=var, hue='class', data=adult_data, multiple='fill', stat='proportion', bins=[0, 10, 30, 45, 60, 70, 99])
plt.title(f"Histogram overlay of {var} and income")
plt.show()

# minimized binning
plt.figure(figsize=(8, 5))
sns.histplot(x=var, hue='class', data=adult_data, multiple='fill', stat='proportion', bins=[0,40,99])
plt.title(f"Histogram overlay of {var} and income")
plt.show()
